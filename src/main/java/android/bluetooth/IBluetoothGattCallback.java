...
@Override public void onListen(int status) throws android.os.RemoteException
{
android.os.Parcel _data = android.os.Parcel.obtain();
android.os.Parcel _reply = android.os.Parcel.obtain();
try {
_data.writeInterfaceToken(DESCRIPTOR);
_data.writeInt(status);
mRemote.transact(Stub.TRANSACTION_onListen, _data, _reply, 0);
_reply.readException();
}
finally {
_reply.recycle();
...
static final int TRANSACTION_onListen = (android.os.IBinder.FIRST_CALL_TRANSACTION + 15);
}
...
public void onListen(int status) throws android.os.RemoteException;
}
